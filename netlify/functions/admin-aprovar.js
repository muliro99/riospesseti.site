...
exports.handler = async (event) => {
  ...
  const result = await githubRequest(
    '/repos/muliro99/riospesseti.site/actions/workflows/publicar-post.yml/dispatches',
    { ref: 'master', inputs: { slug } }
  );

  if (result.status === 204) {
    return { statusCode: 200, body: JSON.stringify({ ok: true, mensagem: 'Publicacao iniciada. O site sera atualizado em instantes.' }) };
  }
  return { statusCode: 500, body: JSON.stringify({ erro: 'Falha ao acionar publicacao.', detalhe: result.body }) };
};
